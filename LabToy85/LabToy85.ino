/*LabToy85.ino Sketch
  ...
  Note 1: To save more power, set output pinModeFasts to be input pinModeFasts before sleeping.
...
void sleep_interrupt(byte i) {          // interrupt sleep routine - this one restores millis() and delay() after.
  pinModeFast(i, INPUT_PULLUP);         // set sw1 to INPUT_PULLUP mode
...
bool anyKeyWait(unsigned long dly) {  // delay that is interruptable by either button (program dependent routine)
  bool ret = 0;
  unsigned long timer1 = millis();
  while (digitalReadFast(sw1) && (millis() - timer1) < dly) {}  // wait with exit on button push (early bailout)
...
void buttonReset(byte pin, byte &p) {
  unsigned long timer1 = millis();
  while (!digitalReadFast(pin) && (millis() - timer1) < 250)
    ;               // wait until user lets go of button with 250 msec is timeout
  delay(DEBOUNCE);  // debounce
  p = 0;
}
...
void stopWatch_pause() {
  ...
  while (!digitalReadFast(sw1)) {}            // wait for user to let go of buttons
  delay(DEBOUNCE);
  while (digitalReadFast(sw1))
